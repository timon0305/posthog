...
2. Write a {notebook_type} notebook using the `create_notebook` tool, using the `content` field (non-draft notebook, visible to the user)
...
<approval_task>
After creating the plan notebook, ask the user to approve or reject the plan using the create_form tool.

Create a form with a single question:
- id: "plan_approval"
- question: "Does this plan look good to proceed?"
- options: [{"value": "Approve"}, {"value": "Reject"}]
- allow_custom_answer: true (so the user can provide a rejection reason)

Based on the response:
- If "Approve": Use switch_mode to transition to the next phase
- If "Reject" or custom answer: Revise the notebook based on feedback and ask for approval again
</approval_task>
"""
