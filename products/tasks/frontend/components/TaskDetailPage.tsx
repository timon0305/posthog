...
import { TaskSegmentList } from './TaskSegmentList'
import { TaskSegmentModal } from './TaskSegmentModal'
...
export function TaskDetailPage({ taskId }: TaskDetailPageProps): JSX.Element {
    ...
    const {
        ...
        isAutoGeneratedTask,
        segmentLinks,
        segmentLinksResponseLoading,
        hasMoreSegments,
        selectedSegment,
    } = useValues(sceneLogic)
    const { setSelectedRunId, runTask, deleteTask, setSelectedSegment, loadMoreSegments } = useActions(sceneLogic)
    ...
    return (
        <SceneContent>
            <ScenePanel>
                    ...
                    <div className="flex flex-col gap-3">
                        ...
                        {isAutoGeneratedTask ? (
                            <>
                                ...
                                {task.last_occurrence_at && (
                                    <div>
                                        <div className="text-xs text-muted mb-1">Last seen</div>
                                        <div className="text-sm">
                                            {dayjs(task.last_occurrence_at).format('MMM D, YYYY HH:mm')}
                                        </div>
                                    </div>
                                )}
                            </>
                            ...
                            <>
                                <div>
                                    ...
                                    <div className="text-sm">{task.repository || 'Not set'}</div>
                                </div>
                                <div>
                ...
                {!isAutoGeneratedTask && runs.length > 0 && (
                    <>
                        <ScenePanelDivider />
                        <ScenePanelInfoSection>
            ...
            <SceneTitleSection
                ...
                actions={
                    <div className="flex items-center gap-2">
                        ...
                        {!isAutoGeneratedTask && !isLatestRunInProgress && (
            ...
            {isAutoGeneratedTask ? (
                <>
                    ...
                    <div className="flex items-center gap-6 px-4 py-3 bg-bg-light border-b">
                        <div className="flex items-center gap-2">
                            <IconPerson className="w-4 h-4 text-muted" />
                            <span className="text-sm">
                                <strong>{humanFriendlyNumber(task.distinct_user_count)}</strong> users affected
                            </span>
                        </div>
                        <div className="flex items-center gap-2">
                    ...
                    <div className="flex-1 overflow-auto p-4">
                        <h3 className="text-sm font-semibold mb-3">Video segments ({task.segment_link_count})</h3>
                        <TaskSegmentList
                            segments={segmentLinks}
                            loading={segmentLinksResponseLoading}
